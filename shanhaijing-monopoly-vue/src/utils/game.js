import { DEFAULT_CHANCE, DEFAULT_FATE, DEFAULT_RULES_TEXT, DEFAULT_START_MONEY, DEFAULT_TOLL_FEE, PLAYER_ANIMALS, DEFAULT_TILES, PASS_GO_BONUS, POSITIONS, } from '../config/game';
export const NUM_TILES = POSITIONS.length;
export function deepClone(value) {
    return JSON.parse(JSON.stringify(value));
}
export function getAuthBase() {
    const overridden = window.PEACHBLOSSOM_AUTH_BASE;
...
export function buildDefaultPlayers(count) {
...
export function normalizeTiles(rawTiles = []) {
    return DEFAULT_TILES.map((tile, index) => {
        const merged = { ...tile, ...(rawTiles[index] ?? {}) };
        const rawOwner = rawTiles[index]?.owner ?? merged.owner;
        const owner = rawOwner === '' || rawOwner === undefined || rawOwner === null
            ? null
...
export function normalizeCards(rawCards, fallback) {
    ...
    const players = Array.isArray(game.players) && game.players.length === playerCount
        ? game.players.map((player, index) => ({
            ...
            money: typeof player?.money === 'number' && player.money >= 0
                ...
                : DEFAULT_START_MONEY,
            position: typeof player?.position === 'number'
...
export function readBoardPresetsRoot(raw) {
    if (!raw) {
        return { byUser: {} };
    }
    try {
...
export function themeGradientForTileLabel(label) {
    const rules = [
...
export function isChanceTile(tile) {
    return (tile.type === 'tile-event' &&
        (String(tile.label).includes('機會') || String(tile.effect || '').includes('機會')));
}
export function isFateTile(tile) {
    return (tile.type === 'tile-event' &&
        (String(tile.label).includes('命運') || String(tile.effect || '').includes('命運')));
}
export function isPurchasableLandTile(tile) {
    return String(tile.effect || '').includes('可購買地塊');
}
export function getTileTollFee(tile) {
...
export function memberLabel(name, email) {
    return name || email || '';
}
export function playerAnimal(index) {
    return PLAYER_ANIMALS[index % PLAYER_ANIMALS.length] ?? '🐾';
}
export function playerTitle(index, member) {
    const name = memberLabel(member?.name, member?.email);
    return `${playerAnimal(index)} ${name || `玩家${index + 1}`}`;
}
export function playerStatusText(index, member) {
    if (!member) {
        return `本機玩家 ${index + 1}`;
    }
