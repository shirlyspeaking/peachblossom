...
import { BOARD_PRESETS_KEY, DEFAULT_START_MONEY, LAND_PRICE, MAX_BOARD_PRESETS_PER_USER, PASS_GO_BONUS, POSITIONS, STORAGE_KEY, TILE_META, } from '../config/game';
import { refreshSession, usePeachAuth } from './usePeachAuth';
import { buildDefaultPlayers, deepClone, defaultState, getTileTollFee, initialTurnLogLine, isChanceTile, isFateTile, isPurchasableLandTile, normalizeState, playerAnimal, playerStatusText, playerTitle, readBoardPresetsRoot, roomsBaseUrl, } from '../utils/game';
...
export function useMonopolyGame() {
    ...
    let toastTimer = null;
    let diceTickTimer = null;
    let diceResultTimer = null;
    const tiles = computed(() => state.value.tiles.map((tile, index) => ({
        ...tile,
        index,
    ...
    function canEditPlayerMoney(index) {
        if (!online.mode)
            return true;
    ...
    function applyServerPayload(data, silentToast = false) {
        if (!data.snapshot)
            return;
    ...
    async function pushOnlineSnapshot(callback) {
        if (!online.mode || !online.roomCode) {
            callback?.(true);
            return;
        }
    ...
    function syncRulesText(value) {
        state.value.rulesText = value;
        debouncedSave();
    }
    function updateTileField(index, field, value) {
        state.value.tiles[index][field] = value;
        debouncedSave();
    }
    function updatePlayerMoney(index, value) {
        if (!state.value.game.players[index])
            return;
    ...
    function showMedalCard(type, title, body, onContinue) {
    ...
    function showBuyLandDialog(html, onChoice) {
    ...
    function applyBoardPresetPayload(preset) {
    ...
    function applyResolvedCardEffect(playerId, title, content, landedPos) {
        const player = state.value.game.players[playerId];
        const teleport = /(立刻\s*)?前往起點|回到起點|移動至起點|到達起點|移至起點|到達\s*「?\s*起點/.test(content) ||
    ...
    function advanceTurnAfterLand(playerId) {
    ...
    function settleOwnedLandToll(playerId, tileIndex) {
        ...
        if (paid < fee) {
            pushTurnLog(`${titleForPlayer(playerId)} 走到「${tile.label}」，應付過路費 ${fee}，但只剩 ${paid}；已全數付給 ${titleForPlayer(owner)}。`);
        }
        else {
    ...
    function tryPurchaseOrFinish(playerId, tileIndex) {
        const tile = state.value.tiles[tileIndex];
        const player = state.value.game.players[playerId];
        ...
        showBuyLandDialog(`是否以 <strong>${LAND_PRICE} 金幣</strong> 購買「<strong>${tile.label}</strong>」？<br>（您目前有 ${player.money} 金幣）`, (accepted) => {
            if (accepted) {
    ...
    function runLandEffects(playerId) {
        ...
        if (isChanceTile(tile)) {
            ...
            showMedalCard('chance', picked?.title.trim() || '牌堆是空的', picked?.content.trim() || '請先新增機會卡。', () => {
        ...
        if (isFateTile(tile)) {
            ...
            showMedalCard('fate', picked?.title.trim() || '牌堆是空的', picked?.content.trim() || '請先新增命運卡。', () => {
                const nextPosition = picked
                    ? applyResolvedCardEffect(playerId, picked.title, picked.content, position)
                    : position;
                tryPurchaseOrFinish(playerId, nextPosition);
    ...
    function rollDice() {
        ...
        if (diceTickTimer)
            window.clearInterval(diceTickTimer);
        if (diceResultTimer)
            window.clearTimeout(diceResultTimer);
    ...
    function enterOnlineMode(roomCode, data) {
        online.mode = true;
    ...
    function toggleSideDrawer(which) {
