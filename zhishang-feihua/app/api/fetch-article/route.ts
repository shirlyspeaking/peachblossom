...
export async function POST(request: NextRequest) {
  ...
  if (!url || typeof url !== "string") {
    return NextResponse.json({ error: "缺少網址", content: "" }, { status: 400 });
  }
...
黑洞的邊界稱為「事件視界」，一旦物質或光線跨越這個邊界，就無法逃脫。黑洞會扭曲周圍的時空，產生強大的引力效應。科學家透過觀察黑洞對周圍恆星和氣體的影響，以及 2019 年公布的首張黑洞影像，逐步揭開黑洞的神秘面紗。`;
    ...
    const res = await fetch(url, {
      headers: {
        "User-Agent": "Mozilla/5.0 (compatible; ZhishangFeihua/1.0)",
      },
    });
    ...
    return NextResponse.json({ error: "讀取失敗", content: "" }, { status: 500 });
  }
}
