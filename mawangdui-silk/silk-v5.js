...
function renderHotspots() {
  ...
  layer.innerHTML = SPOTS.map((spot) => {
    ...
    const label = `<span class="hotspot-label">${spot.short}</span>`;
...
function setMission() {
  ...
  lore.hidden = false;
  lore.innerHTML = `<p class="lore-meaning">圖上共 ${SPOTS.length} 處。右邊先列出全部名稱；尋寶時請先找到這一題的正確位置。</p>` + catalogHtml();
  bindCatalog();
  dock.classList.add("is-reading");
}
...
function loreHtml(spot, withQuiz) {
  ...
  const options = spot.quiz.options
    ...
    .join("");
  const back = `<button type="button" class="btn-ghost" id="btn-catalog">返回目錄</button>`;
  return `
    <div class="kind-row">
      <span>${spot.kind}</span>
    </div>
