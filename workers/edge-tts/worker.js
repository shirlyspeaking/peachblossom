...
const OPENAI_VOICE_MAP = {
  ...
  child: "zh-CN-XiaoshuangNeural", // 童聲（普通話）
};
...
async function getEndpoint(request) {
  ...
  for (var attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      var jwt = parseJwtPayload(data.t);
...
/** Base64URL JWT payload（Edge Translator endpoint 回傳的 t 欄位） */
...
function getAllowedOrigin(request) {
  ...
  try {
    ...
    if (host === "peachspring.cc" || host === "www.peachspring.cc" || host.endsWith(".pages.dev") || host === "localhost") {
      return origin;
    }
  } catch (e) {}
  return "*";
